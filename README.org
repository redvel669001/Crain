...
- `-so` or `--size_opt`: generate a binary ELF file without using fasm, after optimizing the Brainfuck source code, and while attempting to minimize the size of the final binary.
